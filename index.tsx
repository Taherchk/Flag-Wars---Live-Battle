...
// API Key না থাকলে এরর হ্যান্ডলিং
let ai: any = null;
if (process.env.API_KEY) {
    try {
        ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    } catch (e) {
        console.error("Gemini API Initialization failed", e);
    }
}
...
const startMain = () => {
    ...
    // Matter.js লোড না হওয়া পর্যন্ত অপেক্ষা
    if (!Matter) {
    ...
    const startBackgroundMusic = () => {
        if (!soundEnabled || bgMusicNode || !audioCtx) return;
        try {
    ...
    function initGame() {
        ...
        if (mode === 'random-20') filteredCountries = [...COUNTRIES].sort(() => 0.5 - Math.random()).slice(0, 20);
            ...
            if (filteredCountries.length < 2) filteredCountries = [...COUNTRIES].slice(0, 10);
        }
...
//DOMContentLoaded এর পরিবর্তে সরাসরি এবং রিকার্সিভলি চেক করা হচ্ছে
startMain();
